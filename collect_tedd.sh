...
do
    ...
    # grab and massage the answer, replace techniques by TEDD2026
    (echo $j StateSpace ; cat $i | grep STATE_SPACE  | grep -v " -1 " | cut -d ' ' -f 1-4 | while read line; do echo ${line}" TEDD2026"; done ;) > $j-SS.out ;
done ;
