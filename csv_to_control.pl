      ...
      for (my $i=0 ; $i <= $#verdicts ; $i++) {
		  ...
		  if ($globalProperties{$examination}) {
			# GlobalProperties cases : formula name is simply examination
		  	print OUT "FORMULA ".$examination." ".$res." TECHNIQUES ORACLE2026\n";
			print CSV  @fields[0].",".@fields[1].",0,".$res."\n";
		  } else {
			  if ($#verdicts != 0) {
			  	  # ordinary case : 16 formulas
				  print OUT "FORMULA ".$prefix."-".@index[$i]." ".$res." TECHNIQUES ORACLE2026\n";
				  print CSV  @fields[0].",".@fields[1].",".@index[$i].",".$res."\n";
			  } else {
			  	# total failure of all tools in 2026, simulate question mark answer
			  	foreach (@index) {
				  	print OUT "FORMULA ".$prefix."-".$_." "."?"." TECHNIQUES ORACLE2026\n";
